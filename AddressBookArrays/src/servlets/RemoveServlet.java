...
public class RemoveServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (b == null || !b) {
            ...
            String[] arrindex = request.getParameterValues("dzen");
            Process process = new Process();
            if (arrindex == null) {
                response.sendRedirect("remove");
            } else {
                for (int i = arrindex.length - 1; i >= 0; i--) {
                    int index = Integer.parseInt(arrindex[i]);
                    try {
                        process.remove(index, filename);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws
        ...
        if (b == null || !b) {
            ...
            out.println("<form method=\"post\"name=\"dzen\">");
            ...
            out.println("<a class=\"btn btn-link btn-md\"data-toggle=\"dropdown\">Sort  <span class=\"caret\"></span></a>");
            ...
            Process process = new Process();
            try {
                process.viweAll(filename);
                if (process.viweAll(filename).length < 1) {
                    out.println("<p><h3>Please add your first contact !!!</h3></p>");
                } else {
                    for (int i = 0; i < process.viweAll(filename).length; i++) {
                        out.println("<tr><td><input type=\"checkbox\" name =\"dzen\" value =" + arrindex++ + ">" + ar++ + "</td><td>" + process.viweAll(filename)[i].getLastName() + "</td><td>" + process.viweAll(filename)[i].getFirstName() + "</td><td>" + process.viweAll(filename)[i].getEmail() + "</td><td>" + process.viweAll(filename)[i].getPhone() + "</td></tr>");
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
